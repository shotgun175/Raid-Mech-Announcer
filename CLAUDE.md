...
- Library currently covers all 17 raid release groups: **44 gates, 251 mechanics** — all with `notes` populated from Maxroll per-gate guides
...
- Authoritative source for boss HP bar counts is `Npc.json` → `hpBars` in `src-tauri/meter-data/` (upstream repo: github.com/snoww/loa-logs); use the high-`hpBars` boss entry — its `grade` is `"commander"` for most raids but varies per NPC (both Horizon Cathedral bosses are `"boss"`, and Serca mixes `"boss"`/`"commander"`); other entries for the same boss name are low-bar phase or quest variants. Cross-reference with `encounters.json` for the correct raid entry.
...
- Pure-timer mechanics (`timerSecs`, "from pull") announce once per fight via a wall-clock anchored at `mech:fight-start` (1s tick in the overlay page's `startFightClock`; due-window math in `dueTimerMechs` in `$lib/utils/mechanics`). The announce lead reuses `repeatLead`. The tick is gated on live HP, so a wipe's silence stops timer callouts immediately; a mech already past due when first checked (overlay bound mid-fight) is marked fired without speaking. Fired keys share the per-fight `lastFiredKey` set; the clock stops on `mech:encounter-end`.
