...
- Two-tier silence detection in `mech-store.svelte.ts`: `OVERLAY_HIDE_MS = 8_000` clears the HP display only (overlay stays visible showing a "Phase transition…" placeholder using the preserved `gateId`); `GATE_RESET_MS = 60_000` is the safety-net fallback that triggers full encounter-end teardown for the case where `isDead` never fires.
