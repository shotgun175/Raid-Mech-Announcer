...
<div style="display: inline-flex; flex-direction: column; gap: 6px; font-family: Inter, sans-serif;">
  {#if showActiveMech && activeMech}
    <div
      style="background: rgba(167,139,250,0.08); border: 1px solid rgba(167,139,250,{repeatUrgent
        ? '0.5'
        : '0.35'}); border-radius: 24px; padding: 5px 18px; display: flex; align-items: center; justify-content: center; gap: 9px; box-shadow: 0 0 {repeatUrgent
        ? '16px rgba(248,113,113,0.2)'
        : '12px rgba(167,139,250,0.1)'}, 0 4px 16px rgba(0,0,0,0.5);"
    >
      <div
        style="width: 6px; height: 6px; border-radius: 50%; background: #a78bfa; box-shadow: 0 0 6px #a78bfa; flex-shrink: 0; {repeatUrgent
      ...
      <span
        style="font-size: 13px; font-family: ui-monospace, monospace; font-weight: 700; color: {repeatUrgent
          ? '#f87171'
          : '#c4b5fd'}; white-space: nowrap;">↻ {formatTimer(repeatCountdown)}</span
      >
    </div>
  {/if}
  ...
  {#if next && sev}
    <div
      style="background: {urgent
        ? `${sev.color}26`
        : 'rgba(23,23,23,0.85)'}; backdrop-filter: blur(10px); border: 1px solid {urgent
        ? sev.color
        : 'rgba(64,64,64,0.5)'}; border-radius: 24px; padding: 7px 18px; display: flex; align-items: center; justify-content: center; gap: 10px; box-shadow: {urgent
        ? `0 0 20px ${sev.color}60, 0 4px 20px rgba(0,0,0,0.6)`
        : '0 4px 20px rgba(0,0,0,0.5)'}; transition: all 0.3s; user-select: none;"
    >
      <div
