...
>
  ...
  <div style="padding-right: 12px; min-width: 0;">
    <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px;">
      <span style="font-weight: 600; font-size: 13px; color: {isNext ? '#38bdf8' : '#fafafa'}; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{mech.name}</span>
      {#if isNext}
        <span style="font-size: 9px; color: #38bdf8; font-weight: 700; letter-spacing: 0.06em; flex-shrink: 0;">▶ NEXT</span>
      {/if}
    </div>
    {#if mech.notes}
  ...
  <div style="display: flex; justify-content: flex-end;">
    <button onclick={() => onEdit(mech)} title="Edit" style="background: transparent; border: none; cursor: pointer; padding: 4px 7px; border-radius: 4px; font-size: 12px; color: #a3a3a3; transition: color 0.15s, background 0.15s;" onmouseenter={(e) => { (e.currentTarget as HTMLElement).style.color = '#fafafa'; (e.currentTarget as HTMLElement).style.background = '#262626'; }} onmouseleave={(e) => { (e.currentTarget as HTMLElement).style.color = '#a3a3a3'; (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>✎</button>
    <button onclick={() => onDelete(mech.id)} title="Delete" style="background: transparent; border: none; cursor: pointer; padding: 4px 7px; border-radius: 4px; font-size: 12px; color: #f87171; transition: color 0.15s, background 0.15s;" onmouseenter={(e) => { (e.currentTarget as HTMLElement).style.color = '#fca5a5'; (e.currentTarget as HTMLElement).style.background = 'rgba(248,113,113,0.1)'; }} onmouseleave={(e) => { (e.currentTarget as HTMLElement).style.color = '#f87171'; (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>✕</button>
  </div>
</div>
